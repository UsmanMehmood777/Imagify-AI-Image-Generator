...
const Login = () => {
    ...
    <form className='relative bg-white p-10 rounded-xl text-slate-500'>
       ...
       {state !== 'Login' && <div className='border px-6 py-2 flex items-center gap-2
        ...
        <p className='text-sm text-blue-600 my-4 hover:underline cursor-pointer mt-4'>Forgot password?</p>
        <button className='w-full bg-blue-600 text-white py-2.5 rounded-full' type='submit'>{state === 'Login' ? 'Login' : 'Create account'}</button>
        {state === 'Login' ?
        <p className='mt-5 text-center '>Don't have an account? <span className='text-blue-600 cursor-pointer' onClick={() => setState('Sign up')}>Sign up</span></p>
        :
        <p className='mt-5 text-center '>Already have an account? <span className='text-blue-600 cursor-pointer' onClick={() => setState('Login')}>Login</span></p>
        }
        <img onClick={() => setShowLogin(false)} src={assets.cross_icon} alt="" className='w-5 absolute top-5 right-5 cursor-pointer' />
    </form>
